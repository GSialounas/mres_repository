...
At this point we need to decide on our a-posteriori indicator.  There are two approaches we can take.  The first approach would be to  use the Stokes a-posteriori indicator in $\Omega_S$ and the Laplace a-posteriori indicator in $\Omega_L$.  In this way we would obtain an indicator $\eta_{R,K}$ for each $K\in T$ that would look like this:
...
\end{cases}
\end{equation}
The set $\mathcal{E}_{K,\Omega}$ is the set of of edges of $K$ that are interior in the mesh $T$ (i.e. not on the boundary).  Notice that the terms over the edges do not include the contribution from the edges on the interface, $\mathcal{I}$ since these are equal to zero.  

The alternative is to go through the same analysis as we did for the Stokes problem in order to obtain the a-posteriori indicator in \S \ref{a_posteriori_stokes}.  The starting point of our analysis would be the $L^2$-representation of the residual.  This would be similar to what we did for the Stokes equations in (\ref{L2-rep-residual}).   More specifically, the $L^2$-representation of the combined problem residual would be
\begin{eqnarray}\label{L2-rep-residual-mixed}
\langle \ell -L\left(\textbf{u}_T,p_T\right),\left(\textbf{v},q\right) \rangle_Y= &\sum_{K\in T}&\int_{K}\left(\textbf{f}+\Delta \textbf{u}_T\right)\cdot \textbf{v}\nonumber\\-&\sum_{K\in T\cap\Omega_S}&\int_{K}\nabla p_T\cdot \textbf{v}\nonumber\\-&\sum_{E\in \mathcal{E}_{\Omega}\setminus\mathcal{I}}&\int_{E}\llbracket\nabla \textbf{u}_T\rrbracket \cdot \textbf{v}+\sum_{E\in \mathcal{E}_{\Omega_S}\setminus\mathcal{I}}\int_{E}p_TI\textbf{n}_S\cdot \textbf{v}\nonumber\\
-&\sum_{K\in T\cap\Omega_S}&\int_{K}q \nabla \cdot \textbf{u}_T. \nonumber
\end{eqnarray}
Now we follow the same approach that we used for the Stokes a-posteriori indicator.  Namely, we use the restriction operator $Q_T$ defined in (\ref{restrict_operator}) and the bounds introduced in Proposition \ref{verf_prop_bounds} to obtain:

\begin{equation}\small
\begin{aligned}\small
\langle \ell -L\left(\textbf{u}_T,p_T\right),\left(Id-Q_T\right)\left(\textbf{v},q\right) \rangle_Y\leq
&\sum_{K\in T}\left|\left|\textbf{f}+\Delta \textbf{u}_T\right|\right|_K C_{A,2,2}\left(K\right)h_K \left|\left|\nabla\textbf{v}\right|\right|_{2;\widetilde{\omega}_K}\\
+&\sum_{K\in T\cap\Omega_S}\left|\left|\nabla p_T \right|\right|_K C_{A,2,2}\left(K\right)h_K \left|\left|\nabla\textbf{v}\right|\right|_{2;\widetilde{\omega}_K}\\
+&\sum_{E\in \mathcal{E}_{\Omega}}\left|\left|\llbracket\nabla \textbf{u}_T\rrbracket \right|\right|_EC_{A,4,2}\left(K\right)h_E^{1/2} \left|\left|\nabla\textbf{v}\right|\right|_{2;\widetilde{\omega}_E}\\
+&\sum_{E\in \mathcal{E}_{\Omega_S}}\left|\left| p_TI\textbf{n}_S\right|\right|_EC_{A,4,2}\left(K\right)h_E^{1/2} \left|\left|\nabla\textbf{v}\right|\right|_{2;\widetilde{\omega}_E}\\
+&\sum_{K\in T\cap\Omega_S}\left|\left|q\right|\right|_K\left|\left|\nabla \cdot \textbf{u}_T\right|\right|_K\\
\leq c\max \{C_{A,2,2}\left(K\right),C_{A,4,2}\left(E\right)\}\left|\left|\left(\textbf{v},q\right)\right|\right|_X\times\\ \{\sum_{K\in T}h^2_K\left|\left|\textbf{f}+\Delta \textbf{u}_T\right|\right|_K^2+\left|\left|\text{div }\textbf{u}_T\right|\right|^2_K+&\sum_{K\in T\cap\Omega_S}h^2_K\left|\left|\nabla p_T \right|\right|_K^2\\+\sum_{E\in \mathcal{E}_{\Omega}}h_E\left|\left|\llbracket\nabla \textbf{u}_T\rrbracket \right|\right|_E^2 +&\sum_{E\in \mathcal{E}_{\Omega_S}}h_E\left|\left|p_TI\textbf{n}_S\right|\right|_E^2\}^{1/2},
\end{aligned}
\end{equation}
where $c$ is a constant that depends on the shape parameter of our triangulation, $T$ and the norm $\left|\left|\left(\textbf{v},q\right)\right|\right|_X$ is as defined in (\ref{frame_combined}).

The final step is to obtain the a-posteriori error indicator, $\eta_{R,K}$ for the combined problem, which is given by
\begin{equation}\small
\begin{aligned}
\eta_{R,K}^2=h^2_K\left|\left|\textbf{f}_T+\Delta \textbf{u}_T\right|\right|^2_K+h^2_K\left|\left|\nabla p_T\right|\right|^2_K+\left|\left|\text{div} \textbf{u}_T\right|\right|^2_K\\+\frac{1}{2}\sum_{E\in\mathcal{E}_{K,\Omega}\setminus\mathcal{I}}h_E\left|\left| \llbracket \nabla \textbf{u}_T \rrbracket \right|\right|_E^2+\frac{1}{2}\sum_{E\in\mathcal{E}_{K,\Omega_S}\setminus\mathcal{I}}h_E\left|\left|p_T I\textbf{n}_S \right|\right|_E^2,
\end{aligned}
\end{equation}
where the pressure terms are only defined over $\Omega_S$.  Between the two approaches, we will use the first one, whereby we will first decide what model we are using for a given cell and then using the a-posteriori indicator from that model, as shown in (\ref{approach1_indicator}).  The reason for this choice is twofold.  Firstly, it will make for a simpler, cleaner implementation.  Furthermore, it will most likely result in a sharper bound as we have not used a triangle inequality to isolate  the pressure terms in the indicator.
As we mentioned earlier in this section, the first step is to decide on which model we are solving.  We will use a similar algorithm to that used in \cite[\S 6: Algorithm 6.1]{giesselmann2017posteriori}.  

Firstly, we will break our indicator into a modelling and discretisation error $\mathcal{E}_M$ and $\mathcal{E}_D$ respectively.   The modelling error will be given by 
\begin{equation}
